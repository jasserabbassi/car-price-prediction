...
# (uvicorn here), so joblib.load can resolve ensembles serialised by older
...
ENSEMBLE = None
if ENSEMBLE_WEIGHTS and BASE_MODELS:
    try:
        val_r2 = {name: 0.5 for name in ENSEMBLE_WEIGHTS.keys()}
        ENSEMBLE = WeightedEnsemble(BASE_MODELS, val_r2, top_k=None)
        ENSEMBLE.weights = {k: float(v) for k, v in ENSEMBLE_WEIGHTS.items()}
        ENSEMBLE.models = {k: BASE_MODELS[k] for k in ENSEMBLE.weights.keys()}
    except Exception as exc:  # noqa: BLE001
        print(f"[api] WARNING: failed to reconstruct ensemble: {exc}")
